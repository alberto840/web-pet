...
import { UpdateUsuario, GetUsuario } from 'src/app/inventual/state-management/usuario/usuario.action';
...
@Component({
  selector: 'app-inhabilitar-usuario',
  templateUrl: './inhabilitar-usuario.component.html',
  styleUrls: ['./inhabilitar-usuario.component.scss'],
      encapsulation: ViewEncapsulation.None
})
export class InhabilitarUsuarioComponent implements OnInit {
  checked = false;
  ...
  constructor(private dialogRef: MatDialogRef<InhabilitarUsuarioComponent>, @Inject(MAT_DIALOG_DATA) public data: UsuarioModel, public router: Router, private store: Store, public dialogAccess: DialogAccessService, private _snackBar: MatSnackBar, public utils: UtilsService) {
  ...
  async actualizarUsuario() {
    if(this.usuarioUpdated.status === false){
      this.usuarioUpdated.status = true;
    }else{
      this.usuarioUpdated.status = false;
    }
